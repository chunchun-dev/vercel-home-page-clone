import { Button, Image, Link } from '@geist-ui/core'
    ...
    >
        ...
        <section style={{ display: 'flex', gap: 20 }}>
            {NavbarDefaults.navItems.map((navItem, idx) => (
                <>{navItem.link && <Link key={idx} style={{ color: '#939393', fontSize: '0.9rem' }} href={navItem.link}>{navItem.text}</Link>}</>
            ))}
        </section>
        <section style={{ display: 'flex', alignItems: 'center', gap: 19 }}>
            {NavbarDefaults.navActions.map((navAction, idx) => (
                <>{navAction.isButton ? <Button key={idx} type='secondary' color='black' height='32px' width='40px'>{navAction.text}</Button> : <p style={{ color: '#939393', fontSize: '0.9rem' }}>{navAction.text}</p>}</>
            ))}
        </section>
    </div>
  )
