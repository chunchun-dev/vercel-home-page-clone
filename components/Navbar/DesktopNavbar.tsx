    ...
    >
        ...
        <section style={{ display: 'flex', gap: 20 }}>
            {NavbarDefaults.navItems.map((navItem, idx) => (
                <>{navItem.link && <Link style={{ color: '#939393' }} href={navItem.link}>{navItem.text}</Link>}</>
            ))}
        </section>
        <section style={{ display: 'flex', alignItems: 'center', gap: 19 }}>
            {NavbarDefaults.navActions.map((navAction, idx) => (
                <>{navAction.isButton ? <Button type='secondary' color='black' height='32px' width='40px'>{navAction.text}</Button> : <p>{navAction.text}</p>}</>
            ))}
        </section>
    </div>
  )
